...
class CloudflareMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not request.headers.get("cf-connecting-ip"):
            logger.warning(f"Blocked request from {client_ip}: Missing 'cf-connecting-ip' header.")
            raise HTTPException(status_code=403, detail="Direct access forbidden. Use the official domain.")
