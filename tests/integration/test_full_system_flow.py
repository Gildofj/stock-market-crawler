...
from crawler.models.models import Company, Fundamental, MLFeature, StockPrice
...
def test_crawler_to_etl_full_flow(db_session, mocker):
    ...
    mocker.patch.object(engine.b3_spider, 'crawl_ticker', return_value=CrawlResult(
        symbol="FLOW3",
        name="Flow Corp",
        prices=[
            {'time': datetime(2023, 1, 1), 'close': 100.0, 'open': 100.0, 'high': 100.0, 'low': 100.0, 'volume': 1000},
            {'time': datetime(2023, 1, 2), 'close': 110.0, 'open': 110.0, 'high': 110.0, 'low': 110.0, 'volume': 1100}
        ]
    ))
    
    # Simulate enrichment from Fundamentus
    mocker.patch.object(engine.fundamentus_spider, 'crawl_ticker', return_value=CrawlResult(
        symbol="FLOW3",
        p_l=10.0,
        p_vp=2.0,
        dy=5.0,
    ...
    mocker.patch.object(engine.status_spider, 'crawl_ticker', return_value=CrawlResult(
        symbol="FLOW3"
    ))
    ...
    for i in range(65):
        engine.data_service.save_prices(company.id, [StockPriceSchema(
            time=base_date + timedelta(days=i), 
            close=110.0 + i, 
            open=110.0, high=110.0, low=110.0, volume=1000
        )])
    
    etl.generate_features(company.id)
    
    # 5. Verify ML Features
    feature = db_session.query(MLFeature).filter_by(company_id=company.id).order_by(MLFeature.time.desc()).first()
